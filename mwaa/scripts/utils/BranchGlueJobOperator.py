...
class BranchGlueJobOperator(GlueJobOperator):
    ...
    def pre_execute(self, context):
        ...
        #### JOB ARGUMENTS の設定 ####
        base_arguments = dict(self.job_arguments or {})
        ...
        dyn_arguments = {
            ...
            "--catalog": "glue_catalog",  # Glue Catalog + Iceberg用カタログ
            "--warehouse": f"s3://data-platform-lake-yuki-sample/warehouse/",  # Iceberg warehouse
        }
        ...
        self.job_arguments = {**base_arguments, **dyn_arguments}
        print("=" * 50)
        ...
        for key, value in dyn_arguments.items():
            print(f"  {key}: {value}")
        print("最終引数 (job_arguments):")
        for key, value in self.job_arguments.items():
            print(f"  {key}: {value}")
        print("=" * 50)
